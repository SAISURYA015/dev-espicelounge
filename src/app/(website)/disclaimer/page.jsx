  ...
  return (
    <>
      ...
      <main className="mt-16 max-w-7xl mx-auto px-4 py-10 flex gap-8">
        ...
        <aside className="hidden lg:block w-64 sticky top-20 h-fit border-r border-gray-200 pr-4">
          <h2 className="text-lg font-bold mb-4 text-[#152768]">
            Quick Navigation
          </h2>
          <ul className="space-y-2 text-sm">
            {sections.map((sec) => (
              <li key={sec.id}>
                <a
                  href={`#${sec.id}`}
                  className="text-gray-600 hover:text-[#d13b2a] transition-colors"
                >
                  {sec.title}
                </a>
              </li>
